...
def nearest_neighbour_classifier(data):
    ...
    for i in xrange(len(data)):
        if i == 1:
            time_to_finish = np.ceil(((time_end - time_start)*len(data))/60)
            #print 'Time to finish : ', time_to_finish  , 'Minutes'
            print "Time to finish : %.2f Minutes" %time_to_finish
        # if i > 1:
        #     time_now = time()
        #     #print 'Time Remaining : ', time_to_finish - (time_now - time_start)/60, 'Minutes' 
        #     print "Time Remaining : %.2f Minutes" %((time_to_finish - (time_now - time_start)/60))
